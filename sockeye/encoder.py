...
class TransformerEncoder(Encoder):
    ...
    def __init__(self,
        ...
        if self.config.model_type == "ctx_dec_no_share":
            self.layers = [transformer.TransformerEncoderBlock(config, prefix="%s%d_" % (prefix, i),
                                                                     model_type=self.config.model_type, num_layer=i)
                           for i in range(config.num_layers - 1)]
        elif self.config.model_type == C.MODEL_TYPE_AVG_EMB_TOK or self.config.model_type == C.MODEL_TYPE_MAX_EMB_TOK \
                or self.config.model_type == C.MODEL_TYPE_AVG_EMB_ADD or self.config.model_type == C.MODEL_TYPE_MAX_EMB_ADD or self.config.model_type == C.MODEL_TYPE_SUM_EMB_ADD or self.config.model_type == C.MODEL_TYPE_AVG_MAX_EMB_ADD:
            self.layers = [transformer.TransformerNonCtxEncoderBlock(config, prefix="%s%d_" % (prefix, i),
                                                               model_type=self.config.model_type, num_layer=i)
                           for i in range(config.num_layers)]
        elif self.config.model_type == C.MODEL_TYPE_WEAVER:
            self.layers = [transformer.TransformerDocCtxWeaverEncoderBlock(
                config, prefix="%s%d_" % (prefix, i), model_type=self.config.model_type, num_layer=i) for i in
                range(config.num_layers)]
        elif self.config.model_type == C.MODEL_TYPE_AVG_DOC_ATTN:
            self.layers = [transformer.TransformerEncoderDocAttnBlock(config, prefix="%s%d_" % (prefix, i),
                                                                     model_type=self.config.model_type, num_layer=i)
                           for i in range(config.num_layers)]
        else:
            self.layers = [transformer.TransformerEncoderBlockShared(config, prefix="%s%d_" % (prefix, i),
                                                                     model_type=self.config.model_type, num_layer=i)
                           for i in range(config.num_layers - 1)]

        if self.config.model_type == "ctx_dec" or self.config.model_type == "ctx_dec_alt" or self.config.model_type == "ctx_dec_no_share":
            self.layers.append(
                transformer.TransformerEncoderBlock(config, prefix="%s%d_" % (prefix, config.num_layers - 1),
                                                    model_type=self.config.model_type,
                                                    num_layer=config.num_layers - 1))
        elif self.config.model_type == "ctx_enc_dec":
            self.layers.append(
                transformer.TransformerGateEncoderBlock2(config, prefix="%s%d_" % (prefix, config.num_layers - 1)))
        elif not (self.config.model_type == C.MODEL_TYPE_AVG_EMB_TOK or self.config.model_type == C.MODEL_TYPE_MAX_EMB_TOK
                  or self.config.model_type == C.MODEL_TYPE_AVG_EMB_ADD or self.config.model_type == C.MODEL_TYPE_MAX_EMB_ADD or self.config.model_type == C.MODEL_TYPE_SUM_EMB_ADD or self.config.model_type == C.MODEL_TYPE_AVG_DOC_ATTN or self.config.model_type == C.MODEL_TYPE_WEAVER or self.config.model_type == C.MODEL_TYPE_AVG_MAX_EMB_ADD):
            self.layers.append(
                transformer.TransformerGateEncoderBlock(config, prefix="%s%d_" % (prefix, config.num_layers - 1)))
        ...
        if self.config.weaver_conv:

            self.main_conv_weight = mx.sym.Variable('%smain_conv_weight' % prefix)
            self.main_conv_bias = mx.sym.Variable('%smain_conv_bias' % prefix)
        ...
        if self.config.model_type != "ctx_enc" and self.config.model_type != C.MODEL_TYPE_AVG_EMB_TOK \
                ...
                and self.config.model_type != C.MODEL_TYPE_MAX_EMB_ADD and self.config.model_type != C.MODEL_TYPE_SUM_EMB_ADD and self.config.model_type != C.MODEL_TYPE_AVG_DOC_ATTN and self.config.model_type != C.MODEL_TYPE_WEAVER and self.config.model_type != C.MODEL_TYPE_AVG_MAX_EMB_ADD:
    ...
    def encode_with_ctx(self,
        ...
        if not self.config.use_weaver:
            ...
            for i in range(len(self.layers)):
                if (i % 2 == 0 and not self.config.weaver_reverse) or (i % 2 == 1 and self.config.weaver_reverse):
                    bias.append(
                        mx.sym.expand_dims(
                            transformer.get_variable_length_bias(lengths=data_length.repeat(repeats=seq_len, axis=0),
                                                                 # max_length=utils.compute_max_length_pool(ctx_seq_len - 1, self.config.doc_pool_window, self.config.doc_pool_stride) + 1,
                                                                 max_length=ctx_seq_len,
                                                                 num_heads=self.config.attention_heads,
                                                                 fold_heads=True,
                                                                 name='%sbias_%s' % (self.prefix, str(i))), axis=1))
                else:
                    bias.append(
                        mx.sym.expand_dims(transformer.get_variable_length_bias(
                            # lengths=ctx_data_length.repeat(repeats=utils.compute_max_length_pool(ctx_seq_len - 1, self.config.doc_pool_window, self.config.doc_pool_stride) + 1, axis=0),
                            lengths=ctx_data_length.repeat(repeats=ctx_seq_len, axis=0),
                            max_length=seq_len,
                            num_heads=self.config.attention_heads,
                            fold_heads=True,
                            name='%sbias_%s' % (self.prefix, str(i))), axis=1))
        ...
        if self.config.attention_heads != self.config.ctx_attention_heads:
            ctx_bias_small =  mx.sym.expand_dims(transformer.get_variable_length_bias(lengths=ctx_data_length,
                                                                       max_length=ctx_seq_len,
                                                                       num_heads=self.config.ctx_attention_heads,
                                                                       fold_heads=True,
                                                                       name="%sctx_bias_small" % self.prefix), axis=1)
        ...
        if self.config.use_weaver:
            m_data = data.expand_dims(axis=1)
            m_data = mx.sym.repeat(m_data, repeats=ctx_seq_len, axis=2)
            m_data = m_data.reshape(shape=(0, seq_len, ctx_seq_len, -1))
            
            m_ctx_data = ctx_data.expand_dims(axis=1)
            m_ctx_data = mx.sym.repeat(m_ctx_data, repeats=seq_len, axis=1)
            
            data = mx.sym.elemwise_mul(m_data, m_ctx_data)
            
        for i, layer in enumerate(self.layers):
            # (batch_size, seq_len, config.model_size)
            if i == len(self.layers) - 1 and ctx_bias_small is not None:
                data, ctx_data = layer(data, bias, ctx_data, ctx_bias_small)
            elif self.config.model_type == C.MODEL_TYPE_WEAVER:
                data, ctx_data = layer(data, bias, ctx_data, ctx_bias, seq_len, ctx_seq_len, self.config.weaver_reverse)
            else:
        ...
        if self.config.use_weaver:
            ...
            if self.config.weaver_conv:
                ...
                c_data = mx.sym.reshape(data=data, shape=(-1, 1, ctx_seq_len, 512))
                
                ctx_data = mx.sym.Convolution(data=c_data, num_filter=self.config.weaver_conv_kernel, kernel=(self.config.weaver_conv_window, 512),
                                               weight=self.ctx_conv_weight,
                                              bias=self.ctx_conv_bias, pad=(self.config.weaver_conv_window - 1,0))


                # ctx_data = mx.sym.mean(data=ctx_data, axis=3)
                ctx_data = mx.sym.reshape(data=ctx_data, shape=(-1, seq_len, 0, 512))
                if self.config.weaver_sum_type == 'mean':
                    ctx_data = mx.sym.mean(ctx_data, axis=1)
                elif self.config.weaver_sum_type == "max":
                    ctx_data = mx.sym.max(ctx_data, axis=1)

                # data = mx.sym.transpose(data, axes=(0, 2, 1, 3))
                data = mx.sym.reshape(data=data, shape=(-1, 1, seq_len, 512))
                data = mx.sym.Convolution(data=data, num_filter=self.config.weaver_conv_kernel, kernel=(self.config.weaver_conv_window, 512),
                                          weight=self.main_conv_weight,
                                          bias=self.main_conv_bias, pad=(self.config.weaver_conv_window - 1,0))
                # data = mx.sym.mean(data=data, axis=3)
                data = mx.sym.reshape(data=data, shape=(-1, ctx_seq_len, 0, 512))
                if self.config.weaver_sum_type == "mean":
                    data = mx.sym.mean(data, axis=1)
                    # data = mx.sym.transpose(data, axes=(0, 2, 1))
                elif self.config.weaver_sum_type == "max":
                    data = mx.sym.max(data, axis=1)
                    # data = mx.sym.transpose(data, axes=(0, 2, 1))
            else:
                if self.config.weaver_sum_type == "mean":
                    ctx_data = mx.sym.mean(data, axis=1)
                    # ctx_data = mx.sym.Variable('empty')
                    data = mx.sym.mean(data, axis=2)
                elif self.config.weaver_sum_type == "max":
                    ctx_data = mx.sym.max(data, axis=1)
                    data = mx.sym.max(data, axis=2)
