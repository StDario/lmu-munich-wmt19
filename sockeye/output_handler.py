...
def get_output_handler(output_type: str,
    ...
    elif output_type == C.OUTPUT_HANDLER_MULTI_SCORE:
        return MultiScoreOutputHandler(output_stream)
...
class MultiScoreOutputHandler(OutputHandler):
    """
    Output handler to write translation score to a stream.
    ...
    def handle(self,
        ...
        for s in t_output.scores:
            self.stream.write("{:.36} ".format(s))
        self.stream.write('\n')
        self.stream.flush()
...
class PairWithScoreOutputHandler(OutputHandler):
    ...
    def handle(self,
        ...
        :param t_walltime: Total walltime for translation.
        """
        self.stream.write("{}\t{:.36}\n".format(t_output.translation, t_output.score))
        self.stream.flush()
