...
def load_or_create_vocabs(source_paths: List[str],
                          ...
                          pad_to_multiple_of: Optional[int] = None) -> Tuple[List[Vocab], Vocab, bool]:
    """
    ...
    if shared_vocab:
        if source_vocab_path and target_vocab_path:
            ...
            if C.CTX_SYMBOL not in vocab_source:
                vocab_source[C.CTX_SYMBOL] = len(vocab_source)
                missing_ctx_token = True

            if C.CTX_SYMBOL not in vocab_target:
                vocab_target[C.CTX_SYMBOL] = len(vocab_target)
                missing_ctx_token = True
    ...
    return [vocab_source] + vocab_source_factors, vocab_target, missing_ctx_token
