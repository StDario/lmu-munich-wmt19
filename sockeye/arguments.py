...
def add_training_data_args(params, required=False):
    ...
    params.add_argument('--ctx-source',
                        required=required,
                        type=regular_file(),
                        help='Context source side of parallel training data.')

    params.add_argument('--doc-source',
                        required=required,
                        type=regular_file(),
                        help='Document source side of parallel training data.')
...
def add_validation_data_params(params):
    ...
    params.add_argument('--validation-ctx-source',
                        required=True,
                        type=regular_file(),
                        help='Context side of validation data.')

    params.add_argument('--validation-doc-source',
                        required=True,
                        type=regular_file(),
                        help='Context side of validation data.')
...
def add_bucketing_args(params):
    ...
    params.add_argument('--max-doc-seq-len',
                        type=int,
                        default=60,
                        help='Maximum sequence length in tokens for the context.')
...
def add_model_parameters(params):
    ...
    model_params.add_argument('--transformer-feed-forward-num-hidden',
                              ...
                              help='Number of hidden units in transformers feed forward layers. '
                                   'Use "x:x" to specify separate values for encoder & decoder. Default: %(default)s.')
    model_params.add_argument('--transformer-ctx-model-size',
                              type=multiple_values(num_values=2, greater_or_equal=1),
                              default=(512, 512),
                              help='Number of hidden units in transformer layers. '
                                   'Use "x:x" to specify separate values for encoder & decoder. Default: %(default)s.')
    model_params.add_argument('--transformer-ctx-attention-heads',
                              type=multiple_values(num_values=2, greater_or_equal=1),
                              default=(8, 8),
                              help='Number of heads for all self-attention when using transformer layers. '
                                   'Use "x:x" to specify separate values for encoder & decoder. Default: %(default)s.')
    ...
    model_params.add_argument('--freeze-nonctx-params', action="store_true",
                              help='freeze non-context parameters when fine-tuning a baseline model')

    model_params.add_argument('--freeze-ctx-shared-params', action='store_true',
                              help='freeze shared parameters for context and non-context sentences')

    model_params.add_argument('--is-ctx-trg', action="store_true")

    model_params.add_argument('--use-doc-pool', action="store_true",
                              help="summarize context with pooling before attention")
    model_params.add_argument('--doc-pool-window', type=int, default=0, help='document pooling window size')
    model_params.add_argument('--doc-pool-stride', type=int, default=0, help='document pooling window stride')
    ...
    model_params.add_argument('--use-avg-pool', action="store_true", help="use average pooling instead of max")

    model_params.add_argument('--avg-act-type', choices=C.AVG_ACTIVATION_TYPES,
                              default=C.RELU)

    model_params.add_argument('--avg-dropout', type=float, default=0.1)

    model_params.add_argument('--use-weaver', action="store_true",
                              help='a way to summarize and integrate document level context')

    model_params.add_argument('--weaver-reverse', action='store_true', help='reverse the order of the weaving function')
    model_params.add_argument('--weaver-conv', action='store_true', help='apply convolutions after initial weaving')

    model_params.add_argument('--weaver-conv-window', type=int, default=0,
                              help='document weaver convolutional window size')
    model_params.add_argument('--weaver-conv-stride', type=int, default=0,
                              help='document weaver convolutional window stride')

    model_params.add_argument('--weaver-sum-type', type=str, default='mean', choices=['mean', 'max'])
    model_params.add_argument('--weaver-conv-kernel', type=int, default=0, help='document weaver kernel')
...
def add_training_args(params):
    ...
    train_params.add_argument('--load-ctx-params-from-base',
                              action='store_true',
                              help='Initialize contextual parameters from baseline (if possible)')

    train_params.add_argument('--fix-main-encoder-params',
                              action='store_true',
                              help='Fix the main sentence representation parameters in the encoder')

    train_params.add_argument('--fix-main-decoder-params',
                              action='store_true',
                              help='Fix the main sentence representation parameters in the decoder')

    train_params.add_argument('--fix-main-decoder-params-type',
                              type=str,
                              default='relax',
                              choices=['relax', 'strict'],
                              help='Whether to fix the FF and normalization in the decoder and the output weights')
...
def add_score_cli_args(params):
    ...
    params.add_argument("--model", "-m", required=False,
                        help="Model directory containing trained model.")

    params.add_argument("--models", "-ms", required=True, nargs='+',
                        help="Model directory containing trained models.")

    params.add_argument('--checkpoints', '-c',
                               default=None,
                               type=int,
                               nargs='+',
...
def add_inference_args(params):
    ...
    decode_params.add_argument(C.INFERENCE_ARG_CTX_INPUT_LONG,
                               default=None,
                               help='Input file to translate. One sentence per line. '
                                    'If not given, will read from stdin.')
    decode_params.add_argument(C.INFERENCE_ARG_DOC_INPUT_LONG,
                               default=None,
                               help='Input file to translate. One sentence per line. '
                                    'If not given, will read from stdin.')
    ...
    decode_params.add_argument('--max-ctx-input-len',
                               ...
                               default=None,
                               help='Maximum input sequence length. Default: value from model(s).')
    decode_params.add_argument('--max-doc-input-len',
                               type=int,
                               default=None,
                               help='Maximum input sequence length. Default: value from model(s).')
    ...
    decode_params.add_argument('--use-previous-translation',
                               default=False,
                               type=bool,
                               help='Use the previous translation from the model instead of the supplied by the source parameter')

    decode_params.add_argument('--ctx-step-size',
                               default=None,
                               type=int)

    decode_params.add_argument('--doc-step-size',
                               default=None,
                               type=int)
