...
class TrainingModel(model.SockeyeModel):
    ...
    def _initialize(self,
        ...
        # source_words = source.split(num_outputs=self.config.config_embed_source.num_factors,
        #                             axis=1, squeeze_axis=True)[0]
        source_length = utils.compute_lengths(source)
        ctx_source = mx.sym.Variable(C.CTX_SOURCE_NAME)
        # ctx_source_words = ctx_source.split(num_outputs=self.config.config_embed_source.num_factors,
        #                             axis=2, squeeze_axis=True)[0]
        ...
        temp_source = mx.sym.Variable(C.DOC_SOURCE_NAME)
        doc_source_length = utils.compute_lengths(temp_source)
        ...
        data_names = [C.SOURCE_NAME, C.CTX_SOURCE_NAME, C.DOC_SOURCE_NAME, C.TARGET_NAME]
        ...
        self.avg_emb_ff = transformer.TransformerFeedForward(num_hidden=self.config.config_encoder.feed_forward_num_hidden,
        ...
        def sym_gen(seq_lens):
            ...
            """
            source_seq_len, target_seq_len, ctx_source_seq_len, doc_source_seq_len = seq_lens
            ...
            (doc_source_embed,
             ...
             doc_source_embed_seq_len) = self.ctx_embedding_source.encode(temp_source, doc_source_length,
                                                                          doc_source_seq_len)
            ...
            # if self.config.config_encoder.model_type == C.MODEL_TYPE_PRE_AVG:
            doc_source_embed = mx.sym.mean(doc_source_embed, axis=1, keepdims=True)
            doc_source = self.avg_emb_ff(doc_source_embed)

            source_embed = mx.sym.broadcast_add(source_embed, doc_source)
        ...
        if self._bucketing:
            # default_bucket_key = (default_bucket_key[0], default_bucket_key[2], default_bucket_key[1])
    ...
    def get_ctx_token_emb(self, arr, shape):

        hw_scale = 1.
        # hw_scale = np.prod(arr.shape[2:])

        from mxnet import random as rnd
        ...
        rnd.uniform(-scale, scale, out=arr)
        return arr

    def load_params_from_file(self, fname: str, allow_missing_params: bool = False, missing_ctx_token: bool = False):
        """
        ...
        if missing_ctx_token:
            ctx_tok_emb = self.get_ctx_token_emb(mx.nd.zeros(shape=(1, 1024)), shape=(self.params['source_target_embed_weight'].shape[0] + 1, 1))
            embed_weight = mx.nd.concat(self.params['source_target_embed_weight'], ctx_tok_emb, dim=0)
            self.params['source_target_embed_weight'] = embed_weight
            self.params['target_output_wn_scale'] = mx.nd.concat(self.params['target_output_wn_scale'], mx.nd.ones(shape=(1, 1)), dim=0)
            self.params['target_output_bias'] = mx.nd.concat(self.params['target_output_bias'], mx.nd.zeros(shape=(1,)), dim=0)
            # self.params['']
...
class EarlyStoppingTrainer:
    ...
    def fit(self,
        ...
        self._initialize_parameters(existing_parameters, allow_missing_parameters, missing_ctx_token)
        ...
        print("Check the feed forward network")
    ...
    def _initialize_parameters(self, params: Optional[str], allow_missing_params: bool, missing_ctx_token: bool = False):
        ...
        if params is not None:
            ...
            self.model.load_params_from_file(params, allow_missing_params=allow_missing_params, missing_ctx_token=missing_ctx_token)
