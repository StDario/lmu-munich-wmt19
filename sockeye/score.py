...
def get_data_iters_and_vocabs(args: argparse.Namespace,
    ...
    model_configs = [model.SockeyeModel.load_config(os.path.join(m, C.CONFIG_NAME)) for m in args.models]
    ...
    return train_iter, config_data, source_vocabs, target_vocab, model_configs
...
def score(args: argparse.Namespace):
    ...
    with ExitStack() as exit_stack:
        ...
        score_iter, config_data, source_vocabs, target_vocab, model_configs = get_data_iters_and_vocabs(
            args=args,
            model_folder=args.models[0])

        scoring_models = [scoring.ScoringModel(config=model_config,
                                             model_dir=model,
                                             context=context,
                                             ...
                                             softmax_temperature=args.softmax_temperature) for model, model_config, checkpoint in zip(args.models, model_configs, args.checkpoints)]
        ...
        scorer = scoring.Scorer(scoring_models, source_vocabs, target_vocab)
