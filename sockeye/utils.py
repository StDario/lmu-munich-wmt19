...
def load_ctx_from_base_params(fname: str) -> Tuple[Dict[str, mx.nd.NDArray], Dict[str, mx.nd.NDArray]]:
    """
    Loads parameters from a file.
    ...
    for k, v in save_dict.items():
        ...
        if tp == 'arg':
            ...
            if name == "encoder_transformer_final_process_norm_beta":
                arg_params["encoder_transformer_ctx_final_process_norm_beta"] = v
            if name == "encoder_transformer_final_process_norm_gamma":
                arg_params["encoder_transformer_ctx_final_process_norm_gamma"] = v

            elif "encoder_transformer" in name:
                arg_params[name[:22] + "ctx_" + name[22:]] = v

            if "decoder_transformer" in name and "att_enc" in name:
                arg_params[name[:22] + "ctx_" + name[22:]] = v
...
def expand_params_to_fix(fix_encoder: bool = False, num_encoder_layers: int = 0, fix_decoder: bool = False, num_decoder_layers: int = 0, fix_decoder_type: str = 'relax'):

    params_to_fix = []

    if fix_encoder and num_encoder_layers > 0:

        for i in range(num_encoder_layers):
            params_to_fix.append("encoder_transformer_" + str(i) + "_att_self_h2o_weight")
            params_to_fix.append("encoder_transformer_" + str(i) + "_att_self_i2h_weight")
            params_to_fix.append("encoder_transformer_" + str(i) + "_att_self_pre_norm_beta")
            params_to_fix.append("encoder_transformer_" + str(i) + "_att_self_pre_norm_gamma")

            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_h2o_bias")
            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_h2o_weight")
            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_i2h_bias")
            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_i2h_weight")
            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_pre_norm_beta")
            params_to_fix.append("encoder_transformer_" + str(i) + "_ff_pre_norm_gamma")

        params_to_fix.append("encoder_transformer_final_process_norm_beta")
        params_to_fix.append("encoder_transformer_final_process_norm_gamma")

        params_to_fix.append("source_target_embed_weight")

    if fix_decoder and num_decoder_layers > 0:

        for i in range(num_decoder_layers):
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_h2o_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_k2h_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_q2h_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_v2h_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_pre_norm_beta")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_enc_pre_norm_gamma")

            params_to_fix.append("decoder_transformer_" + str(i) + "_att_self_h2o_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_self_i2h_weight")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_self_pre_norm_beta")
            params_to_fix.append("decoder_transformer_" + str(i) + "_att_self_pre_norm_gamma")

            if fix_decoder_type != 'relax':
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_h2o_bias")
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_h2o_weight")
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_i2h_bias")
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_i2h_weight")
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_pre_norm_beta")
                params_to_fix.append("decoder_transformer_" + str(i) + "_ff_pre_norm_gamma")




        if fix_decoder_type != 'relax':
            params_to_fix.append('target_output_bias')
            params_to_fix.append('target_output_wn_scale')
            params_to_fix.append("decoder_transformer_final_process_norm_beta")
            params_to_fix.append("decoder_transformer_final_process_norm_gamma")
