...
class LayerNormalization:
    ...
    def __init__(self,
        ...
        if freeze_params:
            self.scale = mx.sym.BlockGrad(self.scale)
            self.shift = mx.sym.BlockGrad(self.shift)
...
def split_heads(x: mx.sym.Symbol, depth_per_head: int, heads: int, is_weaver: bool = False) -> mx.sym.Symbol:
    """
    ...
    if is_weaver:
        ...
        x = mx.sym.reshape(data=x, shape=(0, 0, -1, heads, depth_per_head))
        # print(x.infer_shape(source=(10, 60), ctx_source=(10, 200))[1])
        # (batch, heads, length, depth/heads)
        x = mx.sym.transpose(data=x, axes=(0, 1, 3, 2, 4))
        # print(x.infer_shape(source=(10, 60), ctx_source=(10, 200))[1])
        x = mx.sym.reshape(data=x, shape=(-3, 0, -1, depth_per_head))
        # print(x.infer_shape(source=(10, 60), ctx_source=(10, 200))[1])
        # (batch * heads, length, depth/heads)
        x = mx.sym.reshape(data=x, shape=(-3, 0, depth_per_head))
        # print(x.infer_shape(source=(10, 60), ctx_source=(10, 200))[1])
        return x
    else:
        # (batch, length, heads, depth_per_head)
...
class MultiHeadAttentionBase:
    ...
    def _attend(self,
                ...
                lengths: Optional[mx.sym.Symbol] = None,
                bias: Optional[mx.sym.Symbol] = None,
                is_weaver: Optional[bool] = False) -> mx.sym.Symbol:
        """
        ...
        # (batch*heads, length, depth/heads)
        queries = split_heads(queries, self.depth_per_head, self.heads, is_weaver)
        keys = split_heads(keys, self.depth_per_head, self.heads, is_weaver)
        values = split_heads(values, self.depth_per_head, self.heads, is_weaver)
...
class MultiHeadSelfAttentionWeaver(MultiHeadAttentionBase):
    """
    Multi-head self-attention. Independent linear projections of inputs serve as
    queries, keys, and values for the attention.
    ...
    def __call__(self,
        ...
        # (batch, max_length, depth)
        # pylint: disable=unbalanced-tuple-unpacking
        queries, keys, values = mx.sym.split(data=combined, num_outputs=3, axis=3)
...
class MultiHeadAttention(MultiHeadAttentionBase):
    ...
    def __init__(self,
        ...
        if freeze_params:
            self.w_q2h = mx.sym.BlockGrad(self.w_q2h)
            self.w_k2h = mx.sym.BlockGrad(self.w_k2h)
            self.w_v2h = mx.sym.BlockGrad(self.w_v2h)
