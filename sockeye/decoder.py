...
class TransformerDecoder(Decoder):
    ...
    def __init__(self,
        ...
        if self.config.model_type == "ctx_dec" or self.config.model_type == "ctx_dec_alt" or self.config.model_type == "ctx_enc_dec" or self.config.model_type == "ctx_dec_no_share":
    ...
    def decode_sequence(self,
        ...
        if self.config.weaver_conv:
            source_encoded_max_length = utils.compute_max_length_pool(source_encoded_max_length,
                                                                      self.config.weaver_conv_window,
                                                                      self.config.weaver_conv_stride)
            ctx_source_encoded_max_length = utils.compute_max_length_pool(ctx_source_encoded_max_length,
                                                                          self.config.weaver_conv_window,
                                                                          self.config.weaver_conv_stride)
        ...
        if self.config.attention_heads != self.config.ctx_attention_heads:
            ctx_source_bias_small = transformer.get_variable_length_bias(lengths=ctx_source_encoded_lengths,
                                                                   max_length=ctx_source_encoded_max_length,
                                                                   num_heads=self.config.ctx_attention_heads,
                                                                   fold_heads=True,
                                                                   name="%sctx_source_bias_small" % self.prefix)

            ctx_source_bias_small = mx.sym.expand_dims(ctx_source_bias_small, axis=1)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if ctx_source_bias_small is not None:
                target = layer(target=target,
                               target_bias=target_bias,
                               source=source_encoded,
                               source_bias=source_bias,
                               ctx_source=ctx_source_encoded,
                               ctx_source_bias=ctx_source_bias_small)
            else:
                target = layer(target=target,
                               target_bias=target_bias,
                               source=source_encoded,
