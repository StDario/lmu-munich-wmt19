...
def extract_embeddings_from_sockeye(f_folder, dim=512):

    # f_folder = params[2]

    fname = f_folder + "/params.best"
    fvocab = f_folder + "/vocab.src.0.json"
    # f_words_out = f_folder + "/words.tsv"
        ...
        for k, v in save_dict.items():
            ...
            if tp == 'arg':
                ...
                if "att_enc_kv2h_weight" in name:
                    ...
                    arg_params[name] = v.asnumpy()
            if tp == 'aux':
                aux_params[name] = v
        return arg_params, aux_params

    model_params, aux_params = load_params(fname)


    # vocab = {}
    temp_vocab = json.loads(open(fvocab).read())
    # vocab = [None] * len(temp_vocab)
    num_words = len(temp_vocab)

    embedding = np.empty((num_words, dim), dtype=np.float32)

    for v, i in temp_vocab.items():
        # vocab[i] = v
        embedding[i] = model_params['source_target_embed_weight'][i]

    return embedding, temp_vocab
...
def create_data_iters_and_vocabs(args: argparse.Namespace,
                                 ...
                                 output_folder: str) -> Tuple['data_io.BaseParallelSampleIter',
                                                              ...
                                                              'data_io.DataConfig',
                                                              List[vocab.Vocab], vocab.Vocab, bool]:
    """
    ...
    if args.prepared_data is not None:
        ...
        if resume_training:
            ...
            source_vocabs, target_vocab, missing_ctx_token = vocab.load_or_create_vocabs(
        ...
        # embeddings, _ = extract_embeddings_from_sockeye("/".join(args.params.split('/')[:-1]), dim=1024)
        embeddings = None

        train_iter, validation_iter, config_data, data_info = data_io.get_training_data_iters(
            sources=sources,
            target=os.path.abspath(args.target),
            ctx_source=os.path.abspath(args.ctx_source),
            doc_source=os.path.abspath(args.doc_source),
            validation_sources=validation_sources,
            validation_target=validation_target,
            validation_ctx_source=str(os.path.abspath(args.validation_ctx_source)),
            validation_doc_source=str(os.path.abspath(args.validation_doc_source)),
        ...
        return train_iter, validation_iter, config_data, source_vocabs, target_vocab, missing_ctx_token
...
def create_encoder_config(args: argparse.Namespace,
    ...
    if args.decoder_only:
        ...
        config_encoder = transformer.TransformerConfig(
            ...
            ctx_model_size=args.transformer_ctx_model_size[0],
            ctx_attention_heads=args.transformer_ctx_attention_heads[0],
            ...
            freeze_nonctx_params=args.freeze_nonctx_params,
            freeze_ctx_shared_params=args.freeze_ctx_shared_params,
            ...
            use_weaver=args.use_weaver,
            weaver_reverse=args.weaver_reverse,
            weaver_conv=args.weaver_conv,
            weaver_conv_window=args.weaver_conv_window,
            weaver_conv_stride=args.weaver_conv_stride,
            weaver_sum_type=args.weaver_sum_type,
            weaver_conv_kernel=args.weaver_conv_kernel,
...
def create_decoder_config(args: argparse.Namespace, encoder_num_hidden: int,
    ...
    if args.decoder == C.TRANSFORMER_TYPE:
        ...
        config_decoder = transformer.TransformerConfig(
            ...
            ctx_model_size=args.transformer_ctx_model_size[1],
            ctx_attention_heads=args.transformer_ctx_attention_heads[1],
            act_type=args.transformer_activation_type,
...
def create_model_config(args: argparse.Namespace,
    ...
    model_config = model.ModelConfig(config_data=config_data,
                                     ...
                                     load_ctx_params_from_base=args.load_ctx_params_from_base,
...
def train(args: argparse.Namespace):
    ...
    args.fixed_param_names.extend(utils.expand_params_to_fix(args.fix_main_encoder_params, args.num_layers[0], args.fix_main_decoder_params,
                               args.num_layers[1], args.fix_main_decoder_params_type))
    ...
    max_seq_len_doc_source = args.max_doc_seq_len
    ...
    max_seq_len_doc_source = max_seq_len_doc_source + C.SPACE_FOR_XOS

    logger.info("Adjusting maximum length to reserve space for a BOS/EOS/CTX marker. New maximum length: (%d, %d, %d)",
    ...
    with ExitStack() as exit_stack:
        ...
        train_iter, eval_iter, config_data, source_vocabs, target_vocab, missing_ctx_token = create_data_iters_and_vocabs(
            args=args,
            max_seq_len_source=max_seq_len_source,
            max_seq_len_target=max_seq_len_target,
            max_seq_len_ctx_source=max_seq_len_ctx_source,
        ...
        trainer.fit(train_iter=train_iter,
                    ...
                    missing_ctx_token=missing_ctx_token)
